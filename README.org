...
Active Node packages for MCP server wiring are managed via =node2nix/node-packages.json= and generated Nix expressions in =node2nix/=. The Nickel file =nickel/packages/node.ncl= is currently an informational manifest and is not wired into the active node2nix pipeline.

When modifying active node2nix inputs, regenerate expressions from repository root:

#+begin_src sh
nix-shell -p node2nix --run "node2nix --input node2nix/node-packages.json --output node2nix/node-packages.nix --composition node2nix/default.nix"
#+end_src
