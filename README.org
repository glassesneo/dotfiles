...
Active Node packages for MCP server wiring are managed via =node-packages/= and generated using bun2nix. The Nickel file =nickel/packages/node.ncl= is currently an informational manifest and is not wired into the active bun2nix pipeline.

When modifying Node package dependencies, regenerate the Nix expressions from repository root:

#+begin_src sh
cd node-packages && bun install && cd .. && bun2nix node-packages node-packages
#+end_src
...
Secrets are defined via sops-nix and referenced as =config.sops.secrets.<key>.path=. Shared declarations live in =modules/toplevel/secrets.nix=, with host binding in =hosts/kurogane/secrets.nix= and encrypted source at =hosts/kurogane/secrets.yaml=.
...
SOPS_AGE_KEY_FILE="$HOME/.config/sops/age/keys.txt" \
  sops edit hosts/kurogane/secrets.yaml
#+end_src
...
Immediate key replacement (single-cutover, no dual-recipient period):
...
# 1) Backup legacy key encrypted to the new recipient.
age -r "$NEW_RECIPIENT" -o "$BACKUP_FILE" "$OLD_KEY_FILE"

# 2) Update .sops.yaml recipient to $NEW_RECIPIENT, then rewrap.
...
# 3) Verify new key-only decrypt, then remove legacy key file.
SOPS_AGE_KEY_FILE="$NEW_KEY_FILE" sops decrypt --extract '["brave-api-key"]' hosts/kurogane/secrets.yaml
rm "$OLD_KEY_FILE"
#+end_src

See =docs/secrets-key-rotation.md= for the full runbook (including validation and rollback).
...
- Module examples: =modules/programs/fd.nix=, =modules/programs/git/default.nix=, =modules/services/aerospace.nix=
