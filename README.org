...
This repository targets macOS on Apple Silicon and is organized around Denix auto-discovery and reusable modules. The primary host is =kurogane= and the user is =neo=.
...
- *Modular architecture* with auto-discovery from =hosts/= and =modules/=
...
Apply user environment only (no system changes):
...
- *OpenCode*: the SketchyBar plugin lives in =modules/programs/opencode/plugins/sketchybar.js= and shares the same SketchyBar plugin entry point.
