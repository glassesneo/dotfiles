...
- Review/debug inspection is owned by `inspector`: review/debug command entrypoints route through `inspector`, which loads concise skills and delegates strict code-review viewpoints to `reviewer1`/`reviewer2` plus pruning/commonization/dead-code review to `pruner`.
