...
- `scout` handles non-source-writing planning/read/report/investigation command workflows such as `/spec`, `/sensei`, `/idea`, `/debug`, `/review`, and `/primary-review`; it may write `.agents/` artifacts and `/tmp` or `/private/tmp` diagnostic sandboxes, but must not modify source, configuration, tests, lockfiles, or git history.
