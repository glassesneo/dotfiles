...
- Review intentionally exists both as `/review` and `reviewer`: `/review` and `/primary-review` are user entrypoints routed through `taskmaster-read`, while `reviewer` is the autonomous subagent for orchestrated review delegation.
- `taskmaster-write` handles implementation-oriented command workflows such as `/impl`, including source edits, validation, triage, and post-implementation review delegation.
- `taskmaster-read` handles read/report command workflows such as `/review` and `/primary-review`; it must not modify source, configuration, tests, or lockfiles.
