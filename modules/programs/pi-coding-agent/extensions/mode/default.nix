  ...
  delib.module {
    ...
    myconfig.always.programs.pi-coding-agent.mode.modes = lib.mapAttrs (_: mode: lib.mapAttrs (_: lib.mkDefault) mode) {
      recon = {
        ...
        instructions = "${judgmentContract} Use ideation-dialogue for open preference-led shaping and intent-elicitation when the user already holds the intended outcome. Investigate evidence directly or delegate when an independent context can materially improve the result. Mesh submissions complete asynchronously; continue useful work, use channels for deliberate cohorts, and retrieve full terminal results with mesh_get. Fetch a single known official URL directly; use authorized research roles for source discovery. Integrate delegated evidence claim by claim and do not mutate repository source.";
      };
      ops = {
        ...
        skillOptIns = ["task-orchestration" "prompt-interface-design" "agent-artifact"];
        instructions = "${judgmentContract} Complete the user's objective directly or through authorized roles when an independent context can materially improve the result. Mesh submissions complete asynchronously; continue useful work, use channels for deliberate cohorts, and retrieve full terminal results with mesh_get. Fetch a single known official URL directly; use authorized research roles for source discovery. Integrate delegated evidence claim by claim and keep source changes within the accepted scope.";
      };
    };
    home.ifEnabled = {
      cfg,
