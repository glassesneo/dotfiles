...
3. Dispatch four separate asynchronous `review-lens` tasks with `mesh_send` concurrently. Record the four returned agent/task IDs, paired with their lenses, as the exact retrieval set for this run. Do not edit source or make overall finding dispositions while any tracked result is missing.
