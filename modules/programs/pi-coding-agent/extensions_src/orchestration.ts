...
const sendParametersFor = (targets: Readonly<Record<string, AgentDefinition>>, profiles: Readonly<Record<string, readonly string[]>>) => {
    ...
    const variants = resolveAuthorizedSelectors(policy, targets).map(route => Type.Object({ agent: Type.Literal(route.selector.agent, { description: route.definition.description }), access: Type.Literal(route.selector.access, { description: route.selector.access === "write" ? "Change source and configuration." : "Leave source and configuration unchanged." }), message }, { additionalProperties: false }));
...
async function startAgentSubmission(deps: OrchestrationDependencies, config: SubagentRuntimeConfig, capturedCaller: ActiveCaller, params: MeshSendParameters & { role: string; completion: CompletionTarget; agentId: string; taskId: string }, signal: AbortSignal | undefined, ctx: ExtensionContext): Promise<AgentSnapshot> {
    ...
    try {
        ...
        const launch = harness.adapter.launch(config, harness.harness, { meshId: caller.meshId, agentId, agentDirectory: prepared.paths.directory, role: params.role, taskPath: taskPaths(config.stateRoot, caller.meshId, taskId).directory, launchEnvelope: envelopePath, epochSnapshot: envelope, cwd: ctx.cwd, ...(resolvedCursorAcpModelId ? { resolvedCursorAcpModelId } : {}) });
        ...
        await publishAgent(config.stateRoot, caller.meshId, prepared.paths, { agentId, epochId: caller.epoch.epochId, role: params.role, selectedProfile, harness: profile.harness, ...(resolvedCursorAcpModelId ? { cursorAcpModelId: resolvedCursorAcpModelId } : {}), cwd: ctx.cwd, roleSnapshot: definition, profileSnapshot: profile, launchEnvelope: envelopePath, tmux, tmuxOwnership: "mesh-hub", capabilities: harness.adapter.capabilities, parentAgentId: caller.agentId, creatorSessionId: ctx.sessionManager.getSessionId(), ...(caller.sessionFile ? { creatorSessionFile: caller.sessionFile } : {}) }, dispatchAuthority(caller, params.role, selectedProfile)); published = true;
        ...
        const internalNames = [params.role, selectedProfile, ...configuredModelDiagnosticNames(profile.models, resolvedCursorAcpModelId)];
