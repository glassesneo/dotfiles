...
import { buildLaunchEnvelope, canonicalJson, projectLaunchEnvelope, validateExecutionProfileConfig, validateLaunchEnvelope, validateOrchestrationConfig, validateOrchestrationReferences, validateRoleCatalog, type AgentDefinition, type AgentLaunchEnvelope, type RoleCatalog } from "./utilities/agent_types.ts";
import { validateModeConfig, type AgentModeConfig, type ExecutionProfileConfig } from "./utilities/mode_types.ts";
...
import { addUsage, emptyUsage, isTerminalAgent, isTerminalTask, type AgentSnapshot, type CompletionReceiptToolName, type CompletionTarget, type PolicyEpoch, type SubagentRuntimeConfig } from "./utilities/orchestration_types.ts";
...
import { displayIdentityForSnapshot, type AgentDisplayIdentity } from "./utilities/orchestration_identity.ts";
...
export interface OrchestrationDependencies { configPath: string; catalogPath?: string; profilePath?: string; modePath?: string; env: NodeJS.ProcessEnv; exec: CommandExecutor; activeCaller?: () => ActiveCaller | undefined; authorityBarrier?: () => Promise<void>; rootLeaseId?: () => string | undefined; natureHandleWords?: () => readonly string[]; sleep?: (ms: number, signal?: AbortSignal) => Promise<void>; now?: () => number }
export interface OrchestrationRegistrationOptions extends Partial<Pick<OrchestrationDependencies, "configPath" | "catalogPath" | "profilePath" | "modePath" | "env">> { setInterval?: (callback: () => void | Promise<void>, delay: number) => unknown; clearInterval?: (timer: unknown) => void; now?: () => number; onAuthorityWait?: () => void; wake?: DirectoryWakeDependencies }
...
export async function loadRoleCatalog(path: string): Promise<RoleCatalog> { try { return validateRoleCatalog(JSON.parse(await readFile(path, "utf8"))); } catch (error) { throw new Error(`Cannot read role catalog ${path}: ${error instanceof Error ? error.message : String(error)}`); } }
export const loadAgentCatalog = loadRoleCatalog;
export async function loadExecutionProfiles(path: string): Promise<ExecutionProfileConfig> { try { return validateExecutionProfileConfig(JSON.parse(await readFile(path, "utf8"))); } catch (error) { throw new Error(`Cannot read execution profiles ${path}: ${error instanceof Error ? error.message : String(error)}`); } }
...
function callerPolicy(caller: ActiveCaller) { return caller.agentId ? caller.envelope?.policies[caller.envelope.role] ?? { targets: {} } : { targets: caller.epoch.directTargets }; }
function callerRoles(caller: ActiveCaller): Record<string, AgentDefinition> { return caller.agentId ? caller.envelope?.roles ?? {} : caller.epoch.roles; }
function callerProfiles(caller: ActiveCaller) { return caller.agentId ? caller.envelope?.profiles ?? {} : caller.epoch.profiles; }
function roleTargets(caller: ActiveCaller): Record<string, AgentDefinition> { const roles = callerRoles(caller); return Object.fromEntries(Object.keys(callerPolicy(caller).targets).map(name => [name, roles[name]!]).filter(([, definition]) => definition)); }
function targetProfiles(caller: ActiveCaller): Record<string, string[]> { return Object.fromEntries(Object.entries(callerPolicy(caller).targets).map(([name, edge]) => [name, [...edge.profiles]])); }
...
function dispatchAuthority(caller: ActiveCaller, targetRole: string, selectedProfile: string) { return { ...requester(caller), ...(caller.sessionFile ? { requesterEndpointSessionFile: caller.sessionFile } : {}), ...(caller.agentId && caller.runtimeId ? { requesterRuntimeId: caller.runtimeId } : {}), epochId: caller.epoch.epochId, policyDigest: caller.epoch.policyDigest, targetRole, selectedProfile }; }
function authorizeDirectRole(caller: ActiveCaller, role: string, action: string): { definition: AgentDefinition; profiles: string[] } { const definition = callerRoles(caller)[role]; const edge = callerPolicy(caller).targets[role]; if (!definition || !edge) throw new Error(`${caller.identity} is not allowed to ${action} role ${role}`); return { definition, profiles: edge.profiles }; }
function resolveSelectedProfile(caller: ActiveCaller, role: string, requestedProfile: string | undefined, action: string) { const edge = authorizeDirectRole(caller, role, action); let selectedProfile: string; if (edge.profiles.length === 1) { if (requestedProfile !== undefined) throw new Error(`Role ${role} has a fixed profile and rejects an explicit profile`); selectedProfile = edge.profiles[0]!; } else { if (requestedProfile === undefined) throw new Error(`Role ${role} requires an explicit profile`); if (!edge.profiles.includes(requestedProfile)) throw new Error(`${caller.identity} is not allowed to ${action} role ${role} with profile ${requestedProfile}`); selectedProfile = requestedProfile; } const profile = callerProfiles(caller)[selectedProfile]; if (!profile) throw new Error(`Profile ${selectedProfile} is outside the immutable caller policy`); return { definition: edge.definition, selectedProfile, profile, authorizedProfiles: edge.profiles }; }
function authorizeTask(caller: ActiveCaller, task: Awaited<ReturnType<typeof readTask>>, action: string): void { if (!caller.agentId) return; if (task.request.requesterAgentId !== caller.agentId || task.request.requesterEndpointId !== caller.endpointId) throw new Error(`${caller.identity} is not allowed to ${action} task ${task.request.taskId}`); }
function authorizeAgent(caller: ActiveCaller, snapshot: AgentSnapshot, action: string): void { if (!caller.agentId) return; if (snapshot.agent.parentAgentId !== caller.agentId) throw new Error(`${caller.identity} is not allowed to ${action} agent ${snapshot.agent.agentId}; caller is not its direct parent`); }
function authorizeReuse(caller: ActiveCaller, snapshot: AgentSnapshot): void { const edge = authorizeDirectRole(caller, snapshot.agent.role, "reuse"); const profile = callerProfiles(caller)[snapshot.agent.selectedProfile]; if (!edge.profiles.includes(snapshot.agent.selectedProfile) || canonicalJson(snapshot.agent.roleSnapshot) !== canonicalJson(edge.definition) || canonicalJson(snapshot.agent.profileSnapshot) !== canonicalJson(profile)) throw new Error(`Agent ${snapshot.agent.agentId} does not match the current immutable role/profile edge`); }
...
export function tuiNoticeText(notice: TuiNotice, expanded = false): string {
    ...
    const agents = notice.payload.confirmed.map(item => `${item.role} ${item.agentId} · ${item.source} · ${item.reason}`);
    return expanded ? [summary, ...agents, `noticeId: ${notice.noticeId}`].join("\n") : summary;
}
export function renderTuiNotice(notice: TuiNotice, expanded: boolean, theme: ExtensionContext["ui"]["theme"]): Component {
    return { invalidate() {}, render(width: number) { const text = theme.fg(notice.kind === "explicit-stop" ? "warning" : "accent", tuiNoticeText(notice, expanded)); return wrapTextWithAnsi(text, Math.max(1, width)).map(line => truncateToWidth(line, Math.max(1, width), "")); } };
}

type MeshSendParameters = { agent?: string; agentId?: string; profile?: string; message: string };
const sendParametersFor = (targets: Readonly<Record<string, AgentDefinition>>, profiles: Readonly<Record<string, readonly string[]>>) => {
    const message = Type.String({ minLength: 1 });
    const variants = Object.entries(targets).map(([name, definition]) => {
        const authorized = profiles[name] ?? [];
...
const waitParameters = Type.Object({ taskIds: Type.Array(Type.String(), { minItems: 1, maxItems: 16, uniqueItems: true }), outputMode: outputMode() }, { additionalProperties: false });
...
type SendSelector = { kind: "new"; agent: string; profile?: string } | { kind: "existing"; agentId: string };
function exactSelector(value: { agent?: string; agentId?: string; profile?: string }, tool: string): SendSelector { if (value.agentId !== undefined) { if (value.agent !== undefined || value.profile !== undefined) throw new Error(`${tool} agentId cannot be combined with agent or profile`); return { kind: "existing", agentId: value.agentId }; } if (value.agent === undefined) throw new Error(`${tool} requires agent or agentId`); return { kind: "new", agent: value.agent, ...(value.profile === undefined ? {} : { profile: value.profile }) }; }
...
function attachDisplayIdentity<T extends object>(details: T, snapshot: AgentSnapshot, words?: readonly string[]): T & { displayIdentity: AgentDisplayIdentity } {
    return { ...details, displayIdentity: displayIdentityForSnapshot(snapshot, words) };
}
...
async function startAgentSubmission(deps: OrchestrationDependencies, config: SubagentRuntimeConfig, capturedCaller: ActiveCaller, params: MeshSendParameters & { role: string; completion: CompletionTarget; requestedProfile?: string; agentId: string; taskId: string }, signal: AbortSignal | undefined, ctx: ExtensionContext): Promise<AgentSnapshot> {
    ...
    const resolved = resolveSelectedProfile(caller, params.role, params.requestedProfile, "start");
    const { definition, selectedProfile, profile, authorizedProfiles } = resolved;
    const reservation = caller.agentId
        ...
        : await reserveNewAgentCapacityWithPressure({ stateRoot: config.stateRoot, meshId: caller.meshId, leaseId: deps.rootLeaseId?.() ?? "", gc: config.gc, exec: deps.exec, tmux: config.tmux, expectedCurrentEpochId: caller.epoch.epochId }, undefined, undefined, undefined, { endpointId: caller.endpointId, endpointSessionFile: caller.sessionFile! });
    ...
    const { agentId, taskId } = params; const harness = resolveHarnessAdapter(config, profile.harness, profile); const tmuxContext = await probeTmux(deps.exec, config.tmux, deps.env); if (!tmuxContext) { await releaseMeshReservation(config.stateRoot, caller.meshId, reservation.reservationId, "tmux unavailable"); throw new Error("Mesh agent start requires a usable current tmux context"); }
    ...
    if (profile.harness === "pi") {
        ...
        const selected = await preflightProfileCandidates({ profile, profileName: selectedProfile, registry: ctx.modelRegistry });
        if (!selected.ok) { await releaseMeshReservation(config.stateRoot, caller.meshId, reservation.reservationId, "profile candidate preflight exhausted"); throw new Error(selected.error); }
    ...
    const roles = callerRoles(caller); const childExtensions = Object.fromEntries(Object.keys(roles).map(name => [name, caller.envelope?.childExtensions[name] ?? [config.popupExtension, config.orchestrationExtension, ...(roles[name]?.childExtensionContributions ?? []), config.childBridgeExtension]]));
    const envelope = caller.envelope
        ? projectLaunchEnvelope(params.role, agentId, caller.envelope, selectedProfile, initialCandidateIndex)
        : buildLaunchEnvelope({ meshId: caller.meshId, agentId, epochId: caller.epoch.epochId, role: params.role, selectedProfile, authorizedProfiles, snapshot: caller.epoch, childExtensions, initialCandidateIndex });
    const prepared = await prepareAgent(config.stateRoot, caller.meshId, { reservationId: reservation.reservationId, agentId, role: params.role, selectedProfile, harness: profile.harness, cwd: ctx.cwd, roleSnapshot: definition, profileSnapshot: profile, launchEnvelope: "pending", epochId: caller.epoch.epochId, provenance: { parentAgentId: caller.agentId, creatorSessionId: ctx.sessionManager.getSessionId(), ...(caller.sessionFile ? { creatorSessionFile: caller.sessionFile } : {}) }, capabilities: harness.adapter.capabilities, authority: dispatchAuthority(caller, params.role, selectedProfile), modelRoute });
    ...
    try {
        ...
        const launch = harness.adapter.launch(config, harness.harness, { meshId: caller.meshId, agentId, agentDirectory: prepared.paths.directory, role: params.role, taskPath: taskPaths(config.stateRoot, caller.meshId, taskId).directory, launchEnvelope: envelopePath, epochSnapshot: envelope, cwd: ctx.cwd });
        ...
        tmux = await launchAgentSession(deps.exec, config.tmux, tmuxContext, { meshId: caller.meshId, epochId: caller.epoch.epochId, agentId, agent: params.role, cwd: ctx.cwd, launch });
        await publishAgent(config.stateRoot, caller.meshId, prepared.paths, { agentId, epochId: caller.epoch.epochId, role: params.role, selectedProfile, harness: profile.harness, cwd: ctx.cwd, roleSnapshot: definition, profileSnapshot: profile, launchEnvelope: envelopePath, tmux, tmuxOwnership: "mesh-hub", capabilities: harness.adapter.capabilities, parentAgentId: caller.agentId, creatorSessionId: ctx.sessionManager.getSessionId(), ...(caller.sessionFile ? { creatorSessionFile: caller.sessionFile } : {}) }, dispatchAuthority(caller, params.role, selectedProfile)); published = true;
        const task = await createTask(config.stateRoot, caller.meshId, agentId, params.message, { ...requester(caller), completion: params.completion }, reservation.reservationId, taskId, dispatchAuthority(caller, params.role, selectedProfile)); taskCreated = true; let snapshot = await readAgentSnapshot(config.stateRoot, caller.meshId, agentId, task.request.taskId);
...
function meshNextAction(caller: ActiveCaller) {
    return caller.agentId
        ? { action: "mesh_wait" as const, condition: "descendant_results_are_required_after_independent_work" as const, taskIds: "all_pending_descendants" as const, calls: "once" as const, keepCallerOpen: true as const }
        : { action: "yield" as const, condition: "only_pending_mesh_work_remains" as const, resumeVia: "mesh-event" as const };
}
function submittedSendResult(raw: AgentSnapshot, caller: ActiveCaller) { const snapshot = sanitizeSnapshot(raw); const details: SubmitDetails = { ...snapshot, accounting: emptyAccounting() }; return { content: [{ type: "text" as const, text: serializeModelVisibleJson({ disposition: "submitted", ...projectMinimalSubmitResult(snapshot), nextAction: meshNextAction(caller) }) }], details }; }
function intervenedSendResult(result: Extract<MeshSendResult, { disposition: "intervened" }>, snapshot: AgentSnapshot, caller: ActiveCaller, words?: readonly string[]) { return { content: [{ type: "text" as const, text: serializeModelVisibleJson({ ...result, nextAction: meshNextAction(caller) }) }], details: attachDisplayIdentity({ ...result }, snapshot, words) }; }
export function createMeshSendTool(deps: OrchestrationDependencies, targets: Readonly<Record<string, AgentDefinition>> = {}, profiles: Readonly<Record<string, readonly string[]>> = {}, stagedActivation?: StagedMeshSendActivation): ToolDefinition { return defineTool({ name: "mesh_send", label: "Send mesh work", description: "Create an authorized task or durably intervene in a direct child’s active task, then follow the returned nextAction.", promptSnippet: "Send mesh work", promptGuidelines: ["After mesh_send, continue useful work independent of pending children. At top level, end the response when only pending mesh work remains; a completion mesh-event resumes the work. As a nested caller, keep the current task open and join all pending descendants with one mesh_wait call."], parameters: sendParametersFor(targets, profiles), executionMode: "sequential", renderCall: renderSendCall, renderResult: (result, options, theme, context) => renderSendResult(result, options, theme, context, deps.natureHandleWords?.()), async execute(toolCallId, rawParams, signal, _update, ctx) { const params = rawParams as MeshSendParameters; const target = exactSelector(params, "mesh_send"); const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); if (target.kind === "new") resolveSelectedProfile(caller, target.agent, target.profile, "start"); const completion = await liveCompletionTarget(config, caller); let storedTarget: AgentSnapshot | undefined; if (target.kind === "existing") { rejectSelfAgent(caller, target.agentId, "send"); storedTarget = await readAgentSnapshot(config.stateRoot, caller.meshId, target.agentId); authorizeAgent(caller, storedTarget, "send to"); authorizeReuse(caller, storedTarget); }
        const mutationCaller = stagedActivation ? await reauthorizeCaller(deps, caller) : caller; await stagedActivation?.(mutationCaller);
        if (target.kind === "new") { const reservation = await reserveNewAgentMeshSendSubmission(config.stateRoot, mutationCaller.meshId, { callerEndpointId: mutationCaller.endpointId, callerEndpointSessionFile: mutationCaller.sessionFile!, toolCallId, canonicalArguments: params }); if (!reservation.created) { const task = await readTask(config.stateRoot, mutationCaller.meshId, reservation.result.taskId).catch(error => (error as NodeJS.ErrnoException).code === "ENOENT" ? undefined : Promise.reject(error)); if (task) return submittedSendResult(await readAgentSnapshot(config.stateRoot, mutationCaller.meshId, task.request.agentId, task.request.taskId), mutationCaller); }
            const snapshot = await startAgentSubmission(deps, config, mutationCaller, { ...params, role: target.agent, requestedProfile: target.profile, completion, agentId: reservation.result.agentId, taskId: reservation.result.taskId }, signal, ctx); return submittedSendResult(snapshot, mutationCaller); }
        authorizeAgent(mutationCaller, storedTarget!, "send to"); authorizeReuse(mutationCaller, storedTarget!); const endpoint = storedTarget!.agent.capabilities.interactiveInterventions ? await resolveRouteEndpoint(config.stateRoot, mutationCaller.meshId, target.agentId) : undefined; const result = await registerStateAwareMeshSend(config.stateRoot, mutationCaller.meshId, { callerEndpointId: mutationCaller.endpointId, callerEndpointSessionFile: mutationCaller.sessionFile!, toolCallId, canonicalArguments: params, ...(endpoint ? { endpoint } : {}), agentId: target.agentId, message: params.message, completion, authority: dispatchAuthority(mutationCaller, storedTarget!.agent.role, storedTarget!.agent.selectedProfile) });
        return result.disposition === "intervened" ? intervenedSendResult(result, storedTarget!, mutationCaller, deps.natureHandleWords?.()) : submittedSendResult(await readAgentSnapshot(config.stateRoot, mutationCaller.meshId, result.agentId, result.taskId), mutationCaller);
    } }); }
export function createMeshGetTool(deps: OrchestrationDependencies): ToolDefinition<typeof getParameters, unknown> { return defineTool({ name: "mesh_get", label: "Get mesh task result", description: "Receive one terminal mesh task result; pending tasks return not-ready with a nextAction instead of a receipt.", promptSnippet: "Receive a terminal mesh result", promptGuidelines: ["Use mesh_get only for a task announced terminal by completion delivery. If mesh_get returns resultAvailable false, follow nextAction instead of calling mesh_get again."], parameters: getParameters, executionMode: "sequential", prepareArguments: args => prepareRetrievalArguments<Static<typeof getParameters>>(args), renderCall: renderGetCall, renderResult: (result, options, theme, context) => renderAgentToolResult(result, options, theme, context, deps.natureHandleWords?.()), async execute(id, params, signal) { const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); const record = await readTask(config.stateRoot, caller.meshId, params.taskId); authorizeTask(caller, record, "inspect"); const snapshot = await readReconciledAgentSnapshot(deps.exec, config.tmux, config.stateRoot, caller.meshId, record.request.agentId, params.taskId); const terminal = Boolean(snapshot.task?.result && isTerminalTask(snapshot.task.status.state)); if (!terminal) return { content: [{ type: "text" as const, text: serializeModelVisibleJson({ taskId: params.taskId, taskState: snapshot.task?.status.state ?? "created", resultAvailable: false, nextAction: meshNextAction(caller) }) }], details: sanitizeSnapshot(snapshot) };
...
type WaitForTerminalSnapshots = (config: SubagentRuntimeConfig, caller: ActiveCaller, taskIds: readonly string[], signal: AbortSignal | undefined) => Promise<AgentSnapshot[]>;
type WaitLifecycleSignal = () => AbortSignal;
async function immediateTerminalSnapshots(config: SubagentRuntimeConfig, caller: ActiveCaller, taskIds: readonly string[], signal?: AbortSignal): Promise<AgentSnapshot[]> { signal?.throwIfAborted(); const snapshots = await Promise.all(taskIds.map(async taskId => { const task = await readTask(config.stateRoot, caller.meshId, taskId); authorizeTask(caller, task, "wait for"); return readAgentSnapshot(config.stateRoot, caller.meshId, task.request.agentId, taskId); })); signal?.throwIfAborted(); if (snapshots.some(snapshot => !snapshot.task?.result || !isTerminalTask(snapshot.task.status.state))) throw new Error("mesh_wait runtime barrier is unavailable for nonterminal tasks"); return snapshots; }
export function createMeshWaitTool(deps: OrchestrationDependencies, waitForTerminal: WaitForTerminalSnapshots = immediateTerminalSnapshots, lifecycleSignal?: WaitLifecycleSignal): ToolDefinition<typeof waitParameters, unknown> { return defineTool({ name: "mesh_wait", label: "Wait for mesh tasks", description: "Wait once for all supplied descendant tasks and return terminal results in input order; available only to nested callers with outbound edges.", promptSnippet: "Await descendant results", promptGuidelines: ["As a nested caller, use one mesh_wait call with all pending descendant task IDs when their results are required and no useful independent work remains."], parameters: waitParameters, executionMode: "sequential", prepareArguments: args => prepareRetrievalArguments<Static<typeof waitParameters>>(args), renderCall: renderWaitCall, renderResult: (result, options, theme, context) => renderWaitResult(result, options, theme, context, deps.natureHandleWords?.()), async execute(id, params, signal) { const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); if (!caller.agentId || !caller.envelope || !caller.sessionFile || !authorizedMeshTools(caller).length) throw new Error("mesh_wait is available only to a durable nested Pi caller with outbound mesh edges"); const endpoint = await resolveRouteEndpoint(config.stateRoot, caller.meshId, "self", caller.agentId); if (!endpoint.online || endpoint.endpointId !== caller.endpointId || endpoint.sessionFile !== caller.sessionFile || !await isLiveMeshEndpointBinding(config.stateRoot, caller.meshId, endpoint)) throw new Error("mesh_wait requires a live durable child endpoint session"); const effectiveSignal = lifecycleSignal ? signal ? AbortSignal.any([signal, lifecycleSignal()]) : lifecycleSignal() : signal; effectiveSignal?.throwIfAborted(); const snapshots = await waitForTerminal(config, caller, params.taskIds, effectiveSignal); effectiveSignal?.throwIfAborted(); const reception = await receiveTerminalSnapshots(config, caller, id, "mesh_wait", canonicalRetrievalArguments(params), snapshots, undefined, effectiveSignal); const details = { tasks: reception.snapshots, accounting: reception.accounting }; const mode = params.outputMode ?? "compact"; return { content: [{ type: "text" as const, text: serializeModelVisibleJson({ tasks: reception.snapshots.map(snapshot => projectMeshRetrievalTask(snapshot, mode)) }, { fullOutputAvailableOnTruncation: mode === "compact" }) }], details, usage: reception.accounting.usage }; } }); }
export function createMeshStopTool(deps: OrchestrationDependencies): ToolDefinition<typeof stopParameters, unknown> { return defineTool({ name: "mesh_stop", label: "Stop mesh task or agent", description: "Stop one task or agent; stopping a task leaves its agent reusable.", promptSnippet: "Stop a mesh task or agent", parameters: stopParameters, executionMode: "sequential", prepareArguments: args => asRecord(args) as Static<typeof stopParameters>, renderCall: renderStopCall, renderResult: (result, options, theme, context) => renderStopResult(result, options, theme, context, deps.natureHandleWords?.()), async execute(id, params) { const hasAgent = params.agentId !== undefined; const hasTask = params.taskId !== undefined; if (hasAgent === hasTask) throw new Error("mesh_stop requires exactly one of agentId or taskId"); if (hasTask && params.reason !== undefined) throw new Error("mesh_stop taskId rejects reason"); const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); let snapshot: AgentSnapshot; let disposition; if (params.agentId) { rejectSelfAgent(caller, params.agentId, "stop"); const stored = await readAgentSnapshot(config.stateRoot, caller.meshId, params.agentId); authorizeAgent(caller, stored, "stop"); const reason = validateStopReason(params.reason ?? "Stopped by mesh peer"); const result = await stopMeshAgentWithDisposition({ stateRoot: config.stateRoot, meshId: caller.meshId, agentId: params.agentId, exec: deps.exec, tmux: config.tmux, source: "peer", reason, requesterEndpointId: caller.endpointId }); snapshot = result.snapshot; disposition = result.disposition; } else { const task = await readTask(config.stateRoot, caller.meshId, params.taskId!); authorizeTask(caller, task, "stop"); const result = await stopMeshTaskWithDisposition({ stateRoot: config.stateRoot, meshId: caller.meshId, taskId: params.taskId!, requesterEndpointId: caller.endpointId }); snapshot = result.snapshot; disposition = result.disposition; } const sanitized = sanitizeSnapshot(snapshot); const details = { ...sanitized, accounting: { claimedTaskIds: [] }, stopDisposition: disposition }; return { content: [{ type: "text" as const, text: serializeModelVisibleJson({ agentId: sanitized.agent.agentId, agent: sanitized.agent.role, agentState: sanitized.status.state, ...(sanitized.task ? { taskId: sanitized.task.request.taskId, taskState: sanitized.task.status.state } : {}), stopDisposition: disposition }) }], details }; } }); }

export function createMeshReportTool(deps: OrchestrationDependencies): ToolDefinition<typeof reportParameters, unknown> { return defineTool({ name: MESH_REPORT_TOOL_NAME, label: "Report mesh progress", description: "Report a decision-relevant progress summary to the direct parent of the active task.", promptSnippet: "Report mesh progress", promptGuidelines: ["Use mesh_report only when the parent requested progress or an intermediate result can change its parallel decision. Do not use it as a heartbeat, final result substitute, or synchronous question."], parameters: reportParameters, executionMode: "sequential", prepareArguments: args => asRecord(args) as Static<typeof reportParameters>, renderCall: renderReportCall, renderResult: renderReportResult, async execute(toolCallId, params) { const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); if (!caller.agentId || !caller.sessionFile) throw new Error("mesh_report is available only to a durable child active task"); const callerSnapshot = await readAgentSnapshot(config.stateRoot, caller.meshId, caller.agentId); const currentTaskId = callerSnapshot.status.activeTaskId; if (!currentTaskId) throw new Error("mesh_report requires an active task"); const task = await readTask(config.stateRoot, caller.meshId, currentTaskId); if (isTerminalTask(task.status.state) || task.request.requesterAgentId === caller.agentId || !task.request.completion) throw new Error("mesh_report requires a nonterminal task with a direct parent completion route"); const parent = await resolveRouteEndpoint(config.stateRoot, caller.meshId, "parent", caller.agentId); if (!parent.online || parent.endpointId !== task.request.completion.endpointId || parent.sessionFile !== task.request.completion.endpointSessionFile) throw new Error("mesh_report direct parent endpoint is unavailable or changed"); const result = await registerMeshReport(config.stateRoot, caller.meshId, { callerEndpointId: caller.endpointId, callerEndpointSessionFile: caller.sessionFile, toolCallId, endpoint: parent, agentId: caller.agentId, taskId: currentTaskId, summary: params.summary, canonicalArguments: params }); const details = attachDisplayIdentity({ reportId: result.reportId, taskId: currentTaskId, state: "queued" as const }, callerSnapshot, deps.natureHandleWords?.()); return { content: [{ type: "text" as const, text: serializeModelVisibleJson({ reportId: result.reportId, taskId: currentTaskId, state: "queued" }) }], details }; } }); }
...
export async function registerOrchestration(pi: ExtensionAPI, options: OrchestrationRegistrationOptions = {}): Promise<boolean> {
    ...
    const initialPolicy = resolvedEnvelope ? resolvedEnvelope.policies[resolvedEnvelope.role] ?? { targets: {} } : { targets: {} };
    const childHasOutboundEdges = Boolean(childAgentId && !isolatedPromptOnly && Object.keys(initialPolicy.targets).length);
    ...
    const registerDispatch = (targets: Record<string, AgentDefinition>, profiles: Record<string, readonly string[]> = {}) => { pi.registerTool(createMeshSendTool(deps, targets, profiles, stagedActivation)); };
    if (!isolatedPromptOnly) { registerDispatch(resolvedEnvelope ? Object.fromEntries(Object.keys(initialPolicy.targets).map(name => [name, resolvedEnvelope!.roles[name]!])) : {}, Object.fromEntries(Object.entries(initialPolicy.targets).map(([name, edge]) => [name, edge.profiles]))); pi.registerTool(createMeshGetTool(deps)); if (childHasOutboundEdges) pi.registerTool(createMeshWaitTool(deps, (...args) => waitForTerminalSnapshots(...args), () => waitLifecycle.signal)); pi.registerTool(createMeshStopTool(deps)); pi.registerTool(createMeshReportTool(deps)); }
    const applyMode = async (event: ActiveModeEvent) => { latestMode = event; if (!sessionContext || childAgentId || !current) return; const restored = branchData<EpochBinding>(sessionContext, POLICY_BINDING); const epoch = await ensurePolicyEpoch(runtime.stateRoot, current.meshId, { mode: event.name, catalog, profiles, callPolicy: runtime.callPolicy, ...(restored?.meshId === current.meshId && restored.mode === event.name ? { restoreEpochId: restored.epochId } : {}) }); current = { ...current, identity: `mode:${event.name}`, epoch, error: undefined }; registerDispatch(roleTargets(current), targetProfiles(current)); if (!restored || restored.epochId !== epoch.epochId) pi.appendEntry(POLICY_BINDING, { schemaVersion: 1, meshId: current.meshId, mode: event.name, epochId: epoch.epochId, policyDigest: epoch.policyDigest } satisfies EpochBinding); };
    ...
    const materializeWithDiagnostics = async (): Promise<boolean> => { try { await requestMaterializationPass(); return true; } catch (error) { if (!maintenance.signal.aborted) sessionContext?.ui.setStatus(PUMP_STATUS, `Mesh completion materialization: ${errorText(error)}`); return false; } };
    const settlePumpWaiters = (error?: unknown) => { const waiters = [...pumpWaiters]; pumpWaiters.clear(); cadence?.setEnabled("wait-delivery", false); for (const waiter of waiters) { if (error === undefined) waiter.resolve(); else waiter.reject(error); } };
    const requestPumpPass = (): Promise<boolean> => { if (pumpPass) return pumpPass; const pass = pumpEvents(); pumpPass = pass; void pass.then(() => { if (pumpPass === pass) pumpPass = undefined; settlePumpWaiters(); }, error => { if (pumpPass === pass) pumpPass = undefined; settlePumpWaiters(error); }); return pass; };
    const nextPumpPass = (signal?: AbortSignal): Promise<void> => new Promise((resolve, reject) => { if (shuttingDown) { reject(new Error("mesh_wait stopped for session shutdown")); return; } if (signal?.aborted) { reject(signal.reason); return; } const waiter = { resolve: () => { signal?.removeEventListener("abort", abort); resolve(); }, reject: (error: unknown) => { signal?.removeEventListener("abort", abort); reject(error); } }; const abort = () => { pumpWaiters.delete(waiter); if (!pumpWaiters.size) cadence?.setEnabled("wait-delivery", false); reject(signal?.reason); }; signal?.addEventListener("abort", abort, { once: true }); pumpWaiters.add(waiter); cadence?.setEnabled("wait-delivery", true); });
    waitForTerminalSnapshots = async (config, caller, taskIds, signal) => { while (true) { if (signal?.aborted) throw signal.reason; const currentCaller = await reauthorizeCaller(deps, caller); if (!currentCaller.agentId || currentCaller.runtimeId !== caller.runtimeId) throw new Error("mesh_wait caller runtime rotated"); await requestPumpPass(); signal?.throwIfAborted(); const snapshots = await Promise.all(taskIds.map(async taskId => { const task = await readTask(config.stateRoot, caller.meshId, taskId); authorizeTask(currentCaller, task, "wait for"); return readAgentSnapshot(config.stateRoot, caller.meshId, task.request.agentId, taskId); })); signal?.throwIfAborted(); if (snapshots.every(snapshot => snapshot.task?.result && isTerminalTask(snapshot.task.status.state))) return snapshots; await nextPumpPass(signal); } };
    const pumpWithDiagnostics = async () => { try { if (await requestPumpPass() && lastPumpError !== undefined) { lastPumpError = undefined; try { sessionContext?.ui.setStatus(PUMP_STATUS, undefined); } catch {} } } catch (error) { if (endpoint && current && !await isLiveMeshEndpointBinding(runtime.stateRoot, current.meshId, endpoint).catch(() => false)) closeWakeHints(); const text = errorText(error).replace(/\s+/gu, " ").trim(); if (text !== lastPumpError) { lastPumpError = text; const diagnostic = `Mesh event pump: ${text}`; try { sessionContext?.ui.setStatus(PUMP_STATUS, diagnostic); } catch {} try { sessionContext?.ui.notify(diagnostic, "error"); } catch {} } } };
