...
const ROOT_BINDING = "mesh-root-binding"; const POLICY_BINDING = "mesh-policy-epoch"; const PARENT_STATUS = "mesh-parent-navigation"; const PUMP_STATUS = "mesh-event-pump"; const WAIT_STATUS = "mesh-armed-wait"; const NOTICE_PUMP_STATUS = "mesh-notice-pump"; const NOTICE_ENTRY = "mesh-tui-notice"; const COMPLETION_DELIVERY_WINDOW_MS = 5_000;
...
export function createMeshSendTool(deps: OrchestrationDependencies, targets: Readonly<Record<string, AgentDefinition>> = {}, profiles: Readonly<Record<string, readonly string[]>> = {}, stagedActivation?: StagedMeshSendActivation): ToolDefinition { return defineTool({ name: "mesh_send", label: "Send mesh work", description: "Create an authorized task or durably intervene in a direct child’s active task, then follow the returned nextAction.", promptSnippet: "Send mesh work", promptGuidelines: ["For a new mesh_send, select an authorized agent and explicitly set access to read when source/configuration changes are not authorized, or write when they are. After submission, continue useful independent work and follow nextAction. Call mesh_wait once when descendant results are required; while it is armed, finish the response when only pending mesh work remains so orchestration can wait without settling the session."], parameters: sendParametersFor(targets, profiles), executionMode: "sequential", renderCall: renderSendCall, renderResult: (result, options, theme, context) => renderSendResult(result, options, theme, context, deps.natureHandleWords?.()), async execute(toolCallId, rawParams, signal, _update, ctx) { const params = rawParams as MeshSendParameters; const target = exactSelector(params, "mesh_send"); const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); if (target.kind === "new") resolveSelectedCapability(caller, target, "start"); const completion = await liveCompletionTarget(config, caller); let storedTarget: AgentSnapshot | undefined; if (target.kind === "existing") { rejectSelfAgent(caller, target.agentId, "send"); storedTarget = await readAgentSnapshot(config.stateRoot, caller.meshId, target.agentId); authorizeAgent(caller, storedTarget, "send to"); authorizeReuse(caller, storedTarget); }
        ...
        if (result.disposition === "intervened") return intervenedSendResult(result, storedTarget!, deps.isWaitArmed?.(mutationCaller), deps.natureHandleWords?.());
