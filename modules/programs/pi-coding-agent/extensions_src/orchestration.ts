...
import { buildLaunchEnvelope, canonicalJson, projectLaunchEnvelope, publicCapability, resolveAuthorizedSelectors, validateExecutionProfileConfig, validateLaunchEnvelope, validateOrchestrationConfig, validateOrchestrationReferences, validateRoleCatalog, type AgentDefinition, type AgentLaunchEnvelope, type RoleCatalog, type RoleSelector } from "./utilities/agent_types.ts";
...
function authorizedSelectorRoutes(caller: ActiveCaller) { return resolveAuthorizedSelectors(callerPolicy(caller), callerRoles(caller)); }
...
function publicCallerLabel(caller: ActiveCaller): string {
    ...
    const selector = caller.envelope?.selfRole.selector;
    return selector ? publicCapability(selector) : "nested caller";
}
function resolveSelectedCapability(caller: ActiveCaller, selector: RoleSelector, action: string) { const route = authorizedSelectorRoutes(caller).find(candidate => candidate.selector.agent === selector.agent && candidate.selector.access === selector.access); if (!route) throw new Error(`${publicCallerLabel(caller)} is not allowed to ${action} capability ${publicCapability(selector)}`); const profile = callerProfiles(caller)[route.profile]; if (!profile) throw new Error("Authorized capability route is outside the immutable caller policy"); return { role: route.role, definition: route.definition, selectedProfile: route.profile, profile, authorizedProfiles: [route.profile] }; }
function authorizeDirectRole(caller: ActiveCaller, role: string, action: string) { const route = authorizedSelectorRoutes(caller).find(candidate => candidate.role === role); if (!route) throw new Error(`${publicCallerLabel(caller)} is not allowed to ${action} this agent capability`); const profile = callerProfiles(caller)[route.profile]; if (!profile) throw new Error("Authorized capability route is outside the immutable caller policy"); return { ...route, profileDefinition: profile }; }
...
function authorizeReuse(caller: ActiveCaller, snapshot: AgentSnapshot): void { const edge = authorizeDirectRole(caller, snapshot.agent.role, "reuse"); if (edge.profile !== snapshot.agent.selectedProfile || canonicalJson(snapshot.agent.roleSnapshot) !== canonicalJson(edge.definition) || canonicalJson(snapshot.agent.profileSnapshot) !== canonicalJson(edge.profileDefinition)) throw new Error(`Agent ${snapshot.agent.agentId} does not match the current immutable capability route`); }
...
type MeshSendParameters = { agent?: string; access?: "read" | "write"; agentId?: string; message: string; profile?: string };
const sendParametersFor = (targets: Readonly<Record<string, AgentDefinition>>, profiles: Readonly<Record<string, readonly string[]>>) => {
    ...
    const policy = { targets: Object.fromEntries(Object.keys(targets).map(role => [role, { profiles: [...(profiles[role] ?? [])] }])) };
    const variants = resolveAuthorizedSelectors(policy, targets).map(route => Type.Object({ agent: Type.Literal(route.selector.agent, { description: route.definition.description }), ...(route.selector.access ? { access: Type.Literal(route.selector.access, { description: route.selector.access === "write" ? "Change source and configuration." : "Leave source and configuration unchanged." }) } : {}), message }, { additionalProperties: false }));
...
function exactSelector(value: MeshSendParameters, tool: string): SendSelector { if (value.agentId !== undefined) { if (value.agent !== undefined || value.access !== undefined || value.profile !== undefined) throw new Error(`${tool} agentId cannot be combined with capability selection`); return { kind: "existing", agentId: value.agentId }; } if (value.agent === undefined) throw new Error(`${tool} requires agent or agentId`); if (value.profile !== undefined) throw new Error(`${tool} does not accept internal profiles`); return { kind: "new", agent: value.agent, ...(value.access === undefined ? {} : { access: value.access }) }; }
...
async function startAgentSubmission(deps: OrchestrationDependencies, config: SubagentRuntimeConfig, capturedCaller: ActiveCaller, params: MeshSendParameters & { role: string; completion: CompletionTarget; agentId: string; taskId: string }, signal: AbortSignal | undefined, ctx: ExtensionContext): Promise<AgentSnapshot> {
    ...
    const resolved = resolveSelectedCapability(caller, { agent: params.agent!, ...(params.access ? { access: params.access } : {}) }, "start");
    const { role, definition, selectedProfile, profile, authorizedProfiles } = resolved;
    if (role !== params.role) throw new Error("Capability route changed before dispatch");
    ...
    try {
        ...
        const internalNames = [params.role, selectedProfile, ...configuredModelDiagnosticNames(profile.models)];
...
export function createMeshSendTool(deps: OrchestrationDependencies, targets: Readonly<Record<string, AgentDefinition>> = {}, profiles: Readonly<Record<string, readonly string[]>> = {}, stagedActivation?: StagedMeshSendActivation): ToolDefinition { return defineTool({ name: "mesh_send", label: "Send mesh work", description: "Create an authorized task or durably intervene in a direct child’s active task, then follow the returned nextAction.", promptSnippet: "Send mesh work", promptGuidelines: ["After mesh_send, continue useful work independent of pending children. At top level, end the response when only pending mesh work remains; a completion mesh-event resumes the work. As a nested caller, keep the current task open and join all pending descendants with one mesh_wait call."], parameters: sendParametersFor(targets, profiles), executionMode: "sequential", renderCall: renderSendCall, renderResult: (result, options, theme, context) => renderSendResult(result, options, theme, context, deps.natureHandleWords?.()), async execute(toolCallId, rawParams, signal, _update, ctx) { const params = rawParams as MeshSendParameters; const target = exactSelector(params, "mesh_send"); const config = await loadOrchestrationConfig(deps.configPath); const caller = await authorized(deps); if (target.kind === "new") resolveSelectedCapability(caller, target, "start"); const completion = await liveCompletionTarget(config, caller); let storedTarget: AgentSnapshot | undefined; if (target.kind === "existing") { rejectSelfAgent(caller, target.agentId, "send"); storedTarget = await readAgentSnapshot(config.stateRoot, caller.meshId, target.agentId); authorizeAgent(caller, storedTarget, "send to"); authorizeReuse(caller, storedTarget); }
        ...
        if (target.kind === "new") { const selected = resolveSelectedCapability(mutationCaller, target, "start"); const reservation = await reserveNewAgentMeshSendSubmission(config.stateRoot, mutationCaller.meshId, { callerEndpointId: mutationCaller.endpointId, callerEndpointSessionFile: mutationCaller.sessionFile!, toolCallId, canonicalArguments: params }); if (!reservation.created) { const task = await readTask(config.stateRoot, mutationCaller.meshId, reservation.result.taskId).catch(error => (error as NodeJS.ErrnoException).code === "ENOENT" ? undefined : Promise.reject(error)); if (task) return submittedSendResult(await readAgentSnapshot(config.stateRoot, mutationCaller.meshId, task.request.agentId, task.request.taskId), mutationCaller); }
            const snapshot = await startAgentSubmission(deps, config, mutationCaller, { ...params, role: selected.role, completion, agentId: reservation.result.agentId, taskId: reservation.result.taskId }, signal, ctx); return submittedSendResult(snapshot, mutationCaller); }
