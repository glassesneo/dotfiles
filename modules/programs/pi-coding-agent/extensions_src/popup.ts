...
export function registerPopupHost(pi: ExtensionAPI): void {
    ...
    pi.events.on(POPUP_OPEN_EVENT, value => { const request = value as PopupOpenRequest; void (async () => { let ownsRoot = false; try { if (!factories.has(request.id)) throw new Error(`Unknown popup view: ${request.id}`); if (request.placement === "push" && active) { request.resolve(await active.open(request.id)); return; } if (opening) throw new Error("A popup root is already opening"); if (request.context.mode !== "tui") throw new Error("Popup views require TUI mode"); opening = true; ownsRoot = true; const result = await request.context.ui.custom<PopupDisposition>((tui, theme, _keys, done) => { active = new PopupStack(tui, theme, request.context, factories, done); void active.open(request.id); return active; }, { overlay: true, overlayOptions: { anchor: "center", width: "70%", minWidth: 60, maxHeight: "80%", margin: 1 } }); request.resolve(result ?? "back"); } catch (error) { request.reject(error); } finally { if (ownsRoot) { active = undefined; opening = false; } } })(); });
}
