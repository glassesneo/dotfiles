...
import { launchEnvelopeDigest, validateLaunchEnvelope, validateOrchestrationConfig } from "./utilities/agent_types.ts";
...
export function registerMeshChildBridge(pi: ExtensionAPI, env: NodeJS.ProcessEnv = process.env, dependencies: MeshChildBridgeDependencies = {}): boolean {
    ...
    const projectUsualIdentity = async () => {
        ...
        try {
            ...
            ctx.ui.setStatus(MESH_CHILD_IDENTITY_STATUS, formatUsualIdentityLine(snapshot, natureHandleWords));
        } catch {}
    };
    const publishActivity = async (phase = activityPhase, heartbeat = false) => { const ctx = bridgeContext; if (!ctx) return; const now = (dependencies.now ?? Date.now)(); if (heartbeat && now - lastHeartbeat < (dependencies.activityHeartbeatMs ?? 2000)) return; const phaseChanged = phase !== activityPhase; if (phaseChanged) { activityPhase = phase; phaseSince = new Date(now).toISOString(); } lastHeartbeat = now; const observedAt = new Date(now).toISOString(); const usage = ctx.getContextUsage?.(); const context = availableContext(usage?.tokens, usage?.contextWindow, reserveTokens, dependencies.contextHeadroomTokens); await (dependencies.publishAgentActivity ?? publishAgentActivity)(stateRoot, meshId, agentId, { runtimeId, phase: activityPhase, acceptingTask: activityPhase === "idle", pendingMessages: ctx.hasPendingMessages?.() ?? false, phaseSince, observedAt, heartbeatAt: observedAt, ...(activityPhase === "compacting" && compactionReason ? { compactionReason } : {}), context }); if (phaseChanged) await projectUsualIdentity(); };
    const queueCompletion = (taskId: string, input: Parameters<typeof finishTask>[3], retirement?: { reason: string; stopped: boolean }) => { pendingCompletion = { taskId, input, deadline: (dependencies.now ?? Date.now)() + (dependencies.completionPersistenceTimeoutMs ?? 30_000), ...(retirement ? { retirement } : {}) }; };
    const persistCompletion = async () => {
        if (!pendingCompletion || completing) return; completing = true; const pending = pendingCompletion; const now = dependencies.now ?? Date.now; const scheduleTimeout = dependencies.setTimeout ?? ((callback: () => void, timeoutMs: number) => globalThis.setTimeout(callback, timeoutMs)); let deadlineTimer: unknown; let timedOut = false;
        try {
            await Promise.race([(async () => { const result = await (dependencies.finishTask ?? finishTask)(stateRoot, meshId, pending.taskId, pending.input, runtimeId); if (pending.retirement) await (dependencies.failAgent ?? failAgent)(stateRoot, meshId, agentId, result.error ?? pending.retirement.reason, result.outcome === "stopped", { expectedRuntimeId: runtimeId }); })(), new Promise<never>((_resolve, reject) => { deadlineTimer = scheduleTimeout(() => { timedOut = true; reject(new Error("Timed out persisting mesh task completion")); }, Math.max(0, pending.deadline - now())); })]);
            if (pendingCompletion === pending) pendingCompletion = undefined; if (activeTaskId === pending.taskId) activeTaskId = undefined; if (pending.retirement) await projectUsualIdentity();
        } catch (error) { if (!timedOut && now() < pending.deadline) throw error; if (!shutdownAfterStoreFailure) { shutdownAfterStoreFailure = true; if (timer !== undefined) { (dependencies.cadenceClearTimeout ?? (value => globalThis.clearTimeout(value as NodeJS.Timeout)))(timer); timer = undefined; } bridgeContext?.shutdown(); } } finally { if (deadlineTimer !== undefined) (dependencies.clearTimeout ?? (value => globalThis.clearTimeout(value as NodeJS.Timeout)))(deadlineTimer); completing = false; }
    };
    ...
    const queueStoppedPromotion = async (): Promise<boolean> => {
        const cancellation = await taskCancellation().catch(() => undefined);
    ...
    const queuePromotionRetirement = async (failure: string): Promise<"exhausted" | "stopped"> => {
        const cancellation = await taskCancellation().catch(() => undefined);
        const stopped = Boolean(cancellation || shuttingDown);
        const reason = cancellation?.reason ?? (shuttingDown ? "Child Pi session shut down" : failure);
                ...
                if (persisted) {
                    ...
                    try {
                        restored = await preflightProfileCandidates({
                            profile,
                            profileName: expectedEnvelope.childId,
