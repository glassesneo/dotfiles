...
export function registerMeshChildBridge(pi: ExtensionAPI, env: NodeJS.ProcessEnv = process.env, dependencies: MeshChildBridgeDependencies = {}): boolean {
    ...
    const persistCompletion = async () => {
        ...
        try {
            ...
            if (pendingCompletion === pending) pendingCompletion = undefined; if (activeTaskId === pending.taskId) { activeTaskId = undefined; activeTaskPrompt = undefined; } if (pending.retirement) await projectUsualIdentity();
    ...
    const promoteAfterError = async (settledStopReason = stopReason, settledErrorMessage = errorMessage): Promise<"promoted" | "exhausted" | "stopped" | "settle" | "held"> => {
        ...
        while (true) {
            ...
            if (await queueStoppedPromotion()) return "stopped";
            pi.sendMessage({ customType: FALLBACK_CONTINUE_CUSTOM_TYPE, content: formatFallbackContinueContent(activeTaskPrompt!), display: false }, { triggerTurn: true });
            return "promoted";
        }
    };
    let lastHoldCount = 0;
