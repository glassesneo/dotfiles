...
const EXECUTION_ROUTE_STATE = "agent-mode-execution-route";
const MODE_STATUS = "agent-mode-identity";
interface ModeState { schemaVersion: 2; mode: string }
interface ExecutionRouteState { schemaVersion: 1; mode: string; models: string[]; route: ProfileRoute }
...
export function registerModeController(pi: ExtensionAPI, configPath = CONFIG): { activeMode: () => string | undefined } {
    ...
    const setIdentity = (ctx: ExtensionContext) => {
        if (activeName) ctx.ui.setStatus(MODE_STATUS, modeIdentityText(activeName, activeRoute?.activeModel ?? modelName(ctx.model), activeRoute?.activeIndex ?? 0));
    };
    const ensureConfig = async (): Promise<void> => {
        config = config ?? await loadAgentModeConfig(configPath);
    };
    ...
    const restoredRoute = (ctx: ExtensionContext, mode: string, execution: ExecutionConfig): ProfileRoute | undefined => {
        const entry = [...ctx.sessionManager.getBranch()].reverse().find(item => item.type === "custom" && item.customType === EXECUTION_ROUTE_STATE) as { data?: unknown } | undefined;
        const data = entry?.data as Partial<ExecutionRouteState> | undefined;
        if (data?.schemaVersion !== 1 || data.mode !== mode) return undefined;
        const route = restoreCompatibleProfileRoute(execution, mode, { profile: mode, models: data.models, route: data.route });
    ...
    const persistRoute = (): void => {
        ...
        if (!activeName || !execution || !activeRoute || activeRoute.activeModel !== execution.models[activeRoute.activeIndex]) return;
        pi.appendEntry(EXECUTION_ROUTE_STATE, { schemaVersion: 1, mode: activeName, models: [...execution.models], route: structuredClone(activeRoute) } satisfies ExecutionRouteState);
    };
    const selectExecution = async (input: { mode: AgentMode; modeName: string; ctx: ExtensionContext; previous: { model: ExtensionContext["model"]; thinking: ReturnType<ExtensionAPI["getThinkingLevel"]>; tools: string[] }; route?: ProfileRoute; persistMode: boolean; reason: ActiveModeReason }): Promise<boolean> => {
        applyingSelection = true;
        let result: Awaited<ReturnType<typeof selectProfileCandidate>>;
        try {
            result = await selectProfileCandidate({
                profile: input.mode.execution,
                profileName: input.modeName,
            ...
            pi.setThinkingLevel(input.mode.execution.thinkingLevel!);
            setTools(input.mode.tools);
        } catch (error) {
            const rollbackFailure = await rollback(input.previous);
            input.ctx.ui.notify(`Mode ${input.modeName}: ${error instanceof Error ? error.message : String(error)}${rollbackFailure ?? ""}`, "error");
            return false;
    ...
    const applyMode = async (name: string, ctx: ExtensionContext, persist: boolean, reason: ActiveModeReason, restore = false): Promise<boolean> => {
        ...
        const restored = restore ? restoredRoute(ctx, name, mode.execution) : undefined;
        ...
        const selected = await selectExecution({ mode, modeName: name, ctx, previous: { model: ctx.model, thinking: pi.getThinkingLevel(), tools: pi.getActiveTools() }, route: restored, persistMode: persist, reason });
        if (!selected && restore) {
    ...
    pi.on("agent_settled", async (_event, ctx) => {
        ...
        try {
            promotion = await promoteProfileCandidate({ profile: execution, profileName: activeName!, route: activeRoute, registry: ctx.modelRegistry, tokens: usage?.tokens, activate: async model => {
                if (shuttingDown) return false;
                const selected = await pi.setModel(model);
