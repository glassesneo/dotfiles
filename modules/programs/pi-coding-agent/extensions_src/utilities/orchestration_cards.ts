...
function isRenderableAgentSnapshot(value: unknown): value is AgentSnapshot {
    ...
    if (typeof (request as { prompt?: unknown }).prompt !== "string") return false;
...
function collapsedAgentCard(theme: Theme, snapshot: AgentSnapshot, argsPrompt?: string, handles?: Map<string, string>): string {
    ...
    const agentType = agentTypeText(theme, snapshot.agent.agent);
    ...
    const line1 = joinParts([handle, agentType, agentState, taskState]);
    const summary = snapshot.task ? promptSummary(snapshot.task.request.prompt) : "No task";
    const prompt = snapshotPrompt(snapshot, argsPrompt);
    const lines = [line1, theme.fg("muted", summary)];
...
function expandedAgentCard(theme: Theme, snapshot: AgentSnapshot, argsPrompt?: string, handles?: Map<string, string>): string {
    ...
    const lines = [
        ...
        labeled(theme, "agent", agentTypeText(theme, snapshot.agent.agent)),
        labeled(theme, "agentState", agentStateText(theme, snapshot.status.state)),
        labeled(theme, "summary", task ? promptSummary(task.request.prompt) : "No task"),
    ];
    if (task) {
...
export function renderSubmitCall(
    args: { agent?: string; agentId?: string; prompt: string },
    theme: Theme,
    context: CardRenderContext,
): Component {
    const title = theme.fg("accent", "subagent_submit");
    const target = args.agent !== undefined
        ? joinParts(["1 new agent", agentTypeText(theme, args.agent)])
        : joinParts(["1 existing agent", args.agentId ? shortId(args.agentId) : undefined]);
    const body = [
        joinParts([target, theme.fg("muted", promptSummary(args.prompt))]),
...
export function renderRunCall(
    ...
    const target = args.agent !== undefined
        ? joinParts(["1 new agent", agentTypeText(theme, args.agent)])
        : joinParts(["1 existing agent", args.agentId ? shortId(args.agentId) : undefined]);
    const body = [joinParts([target, theme.fg("muted", promptSummary(args.prompt))]), callPromptText(theme, args.prompt, context.expanded === true)].join("\n");
...
export function renderRunResult(
    ...
    const target = context.args?.agent !== undefined ? "NEW AGENT" : "EXISTING AGENT";
