...
export interface ActiveModeEvent { schemaVersion: 2; name: string; reason: ActiveModeReason }

export function emitActiveMode(pi: ExtensionAPI, name: string, reason: ActiveModeReason): void {
    pi.events.emit(ACTIVE_MODE_EVENT, { schemaVersion: 2, name, reason } satisfies ActiveModeEvent);
}
export function validateActiveModeEvent(value: unknown): ActiveModeEvent {
    ...
    const unknown = Object.keys(raw).filter(key => !["schemaVersion", "name", "reason"].includes(key));
    if (unknown.length) throw new Error(`active-mode event contains unknown keys: ${unknown.join(", ")}`);
    if (raw.schemaVersion !== 2) throw new Error("Unsupported active-mode event schemaVersion");
    ...
    return { schemaVersion: 2, name: raw.name, reason: raw.reason };
}
