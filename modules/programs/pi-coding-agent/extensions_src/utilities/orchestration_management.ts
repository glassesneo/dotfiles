...
import { agentPaths, failAgent, listOriginAgents, markAgentCleanupPending, markAgentStoppingWithDisposition, readAgentSnapshot, requestTaskCancellation, restoreAgentAfterStopFailure } from "./orchestration_store.ts";
import { isTerminalAgent, isTerminalTask, tmuxOwnership, type AgentSnapshot } from "./orchestration_types.ts";
import { inspectAgentTmux, originHubName, stopAgentSession, stopOriginHub, type CommandExecutor, type TmuxContext } from "./orchestration_tmux.ts";
...
export class OriginAgentDiscovery {
    readonly options: { stateRoot: string; originSessionId: string; authorizedAgents: readonly string[]; exec: CommandExecutor; tmux: string };
    constructor(options: { stateRoot: string; originSessionId: string; authorizedAgents: readonly string[]; exec: CommandExecutor; tmux: string }) { this.options = options; }
    async refresh(): Promise<{ agents: AgentSnapshot[]; malformedCount: number }> {
        const allowed = new Set(this.options.authorizedAgents);
        const found = (await listOriginAgents(this.options.stateRoot, this.options.originSessionId)).filter(item => allowed.has(item.agent.agent));
...
interface TerminateAgentOptions { stateRoot: string; agentId: string; originSessionId: string; exec: CommandExecutor; tmux: string; authorizedAgents?: readonly string[]; callerIdentity?: string }
