...
async function ensureConfirmedExplicitStopNotice(options: TerminateAgentOptions, snapshot: AgentSnapshot, request: AgentStopRequest): Promise<void> {
    ...
    await createExplicitStopNotice(options.stateRoot, options.meshId, { endpointId, requesterEndpointId: request.requesterEndpointId, payload: { stopRequestId: request.stopRequestId, agentId: snapshot.agent.agentId, agent: snapshot.agent.definitionSnapshot.selector.agent, access: snapshot.agent.definitionSnapshot.selector.access, ...(snapshot.task?.request.purpose ? { purpose: snapshot.task.request.purpose } : {}), source: request.source, reason: request.reason } }).catch(() => undefined);
}
