...
export async function reserveNewAgentCapacityWithPressure(options: GcOptions, requestedReservationId?: string, requesterGuard?: () => Promise<void>, requester?: { agentId: string; runtimeId: string; endpointId?: string; endpointSessionFile?: string }, expectedBinding?: ExpectedEndpointBinding): Promise<BudgetReservation> {
    ...
    const retryReservation = async (): Promise<BudgetReservation | undefined> => {
        try { return await reserveMeshCapacity(options.stateRoot, options.meshId, "new-agent-task", undefined, reservationId, options.expectedCurrentEpochId, undefined, expectedBinding ?? (requester?.endpointId && requester.endpointSessionFile ? { endpointId: requester.endpointId, endpointSessionFile: requester.endpointSessionFile } : undefined)); }
