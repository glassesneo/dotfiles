...
export async function pollMeshEvents(stateRoot: string, meshId: string, endpoint: MeshEndpoint, options: CompletionMaterializationOptions = {}): Promise<MeshEvent[]> { validateEndpoint(endpoint, meshId, endpoint.endpointId); return withMeshLock(stateRoot, meshId, async () => { const durableRaw = await optional<unknown>(endpointPath(stateRoot, meshId, endpoint.endpointId)); const durable = durableRaw === undefined ? undefined : validateEndpoint(durableRaw, meshId, endpoint.endpointId); if (!durable || !durable.online || durable.sessionId !== endpoint.sessionId || durable.sessionFile !== endpoint.sessionFile) return []; await evaluateCompletionRoutes(stateRoot, meshId, options); const events = (await records(meshPaths(stateRoot, meshId).events, "retry-")).map(value => validateEvent(value, meshId)); return events.filter(event => event.endpointId === durable.endpointId && event.endpointSessionFile === durable.sessionFile && event.state !== "acknowledged"); }); }
