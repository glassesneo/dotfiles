...
export interface SubagentRuntimeConfig { schemaVersion: 3; stateRoot: string; tmux: string; childExtensions: string[]; harnesses: Record<string, HarnessRuntimeConfig> & { pi: HarnessRuntimeConfig }; maxDepth: number; bridgeReadyTimeoutMs?: number }
...
export interface AgentRecord extends AgentLineage { schemaVersion: 1; agentId: string; profile: string; purpose: string; harness: string; cwd: string; createdAt: string; profileSnapshot: AgentProfile; tmux: TmuxAgentReference; capabilities: NativeCapabilities }
export interface AgentStatus { schemaVersion: 1; agentId: string; state: AgentState; activeTaskId?: string; latestTaskId?: string; bridgeReady: boolean; agentUsage: Usage; accountedTaskIds: string[]; updatedAt: string; exitReason?: string }
...
export function validateSubagentRuntimeConfig(value: unknown): SubagentRuntimeConfig { const root = object(value, "subagent config"); if (root.schemaVersion !== 3) throw new Error("Unsupported subagent config schemaVersion"); const harnesses = object(root.harnesses, "harnesses"); const parsed: Record<string, HarnessRuntimeConfig> = {}; for (const [id, raw] of Object.entries(harnesses)) parsed[id] = { command: nonBlank(object(raw, `harnesses.${id}`).command, `harnesses.${id}.command`) }; if (!parsed.pi) throw new Error("harnesses.pi must be configured"); if (!Number.isInteger(root.maxDepth) || (root.maxDepth as number) < 0) throw new Error("maxDepth must be a non-negative integer"); return { schemaVersion: 3, stateRoot: nonBlank(root.stateRoot, "stateRoot"), tmux: nonBlank(root.tmux, "tmux"), childExtensions: strings(root.childExtensions, "childExtensions"), harnesses: parsed as SubagentRuntimeConfig["harnesses"], maxDepth: root.maxDepth as number, ...(Number.isInteger(root.bridgeReadyTimeoutMs) ? { bridgeReadyTimeoutMs: root.bridgeReadyTimeoutMs as number } : {}) }; }
