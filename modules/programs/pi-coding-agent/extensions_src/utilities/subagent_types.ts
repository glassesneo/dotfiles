...
export interface SubagentRuntimeConfig { schemaVersion: 4; stateRoot: string; tmux: string; historyViewerExtension: string; childExtensions: string[]; harnesses: Record<string, HarnessRuntimeConfig> & { pi: HarnessRuntimeConfig }; maxDepth: number; bridgeReadyTimeoutMs?: number }
...
export function validateSubagentRuntimeConfig(value: unknown): SubagentRuntimeConfig { const root = object(value, "subagent config"); if (root.schemaVersion !== 4) throw new Error("Unsupported subagent config schemaVersion"); const harnesses = object(root.harnesses, "harnesses"); const parsed: Record<string, HarnessRuntimeConfig> = {}; for (const [id, raw] of Object.entries(harnesses)) parsed[id] = { command: nonBlank(object(raw, `harnesses.${id}`).command, `harnesses.${id}.command`) }; if (!parsed.pi) throw new Error("harnesses.pi must be configured"); if (!Number.isInteger(root.maxDepth) || (root.maxDepth as number) < 0) throw new Error("maxDepth must be a non-negative integer"); return { schemaVersion: 4, stateRoot: nonBlank(root.stateRoot, "stateRoot"), tmux: nonBlank(root.tmux, "tmux"), historyViewerExtension: nonBlank(root.historyViewerExtension, "historyViewerExtension"), childExtensions: strings(root.childExtensions, "childExtensions"), harnesses: parsed as SubagentRuntimeConfig["harnesses"], maxDepth: root.maxDepth as number, ...(Number.isInteger(root.bridgeReadyTimeoutMs) ? { bridgeReadyTimeoutMs: root.bridgeReadyTimeoutMs as number } : {}) }; }
