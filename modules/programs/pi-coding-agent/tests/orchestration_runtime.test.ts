...
import { bindMeshEndpoint, materializeMeshCompletionEvents, readMeshEndpoint, registerMeshSignal, resolveRouteEndpoint, setMeshEndpointOffline } from "../extensions_src/utilities/orchestration_events.ts";
...
const REQUIRED_PEER_CAPABILITIES = ["mesh_submit", "mesh_get", "mesh_wait", "mesh_stop", "mesh_signal"] as const;
...
void test("core tool schemas and execution require exactly one selector at each single-target boundary", async () => {
    ...
    const submit = createMeshSubmitTool(deps, targets, { worker: ["pi-default", "cursor-write"] });
    assert.equal(Value.Check(submit.parameters, { agent: "worker", profile: "pi-default", prompt: "work" }), true);
    assert.equal(Value.Check(submit.parameters, { agent: "worker", prompt: "work" }), false);
    assert.equal(Value.Check(submit.parameters, { agent: "worker", profile: "cursor-write", prompt: "work", channel: "A" }), false);
    const fixed = createMeshSubmitTool(deps, { explorer: settledAgentDefinition("explorer") }, { explorer: ["pi-default"] });
    assert.equal(Value.Check(fixed.parameters, { agent: "explorer", prompt: "inspect" }), true);
    assert.equal(Value.Check(fixed.parameters, { agent: "explorer", profile: "pi-default", prompt: "inspect" }), false);
    assert.equal(Value.Check(submit.parameters, { agentId: "agent-id", prompt: "work" }), true);
    assert.equal(Value.Check(submit.parameters, { profile: "cursor-write", prompt: "work" }), false);
    assert.equal(Value.Check(submit.parameters, { agent: "unknown", profile: "pi-default", prompt: "work" }), false);
    await assert.rejects(submit.execute("call", { agent: "worker", agentId: "id", profile: "pi-default", prompt: "work" }, undefined, undefined, {} as never), /cannot be combined/u);
    const get = createMeshGetTool(deps); const wait = createMeshWaitTool(deps);
    await assert.rejects(get.execute("call", { taskId: "task", agentId: "agent" }, undefined, undefined, {} as never), /exactly one/u);
    await assert.rejects(get.execute("call", { taskId: "task", debug: true }, undefined, undefined, {} as never), /debug.*agentId/u);
    await assert.rejects(get.execute("call", { agentId: "agent", debug: true, outputMode: "full" }, undefined, undefined, {} as never), /outputMode.*debug/u);
    assert.equal(Value.Check(get.parameters, { taskId: "task", outputMode: "compact" }), true); assert.equal(Value.Check(get.parameters, { taskId: "task", outputMode: "full" }), true); assert.equal(Value.Check(get.parameters, { taskId: "task", outputMode: "other" }), false);
...
void test("mesh_submit profile selector exposes only authorized names and rejects forged profiles before launch", async () => withRoot("mesh-profile-schema-", async root => {
    ...
    const tool = createMeshSubmitTool(deps, { worker: settledAgentDefinition("worker") }, { worker: ["pi-default", "cursor-write"] }); assert.equal(Value.Check(tool.parameters, { agent: "worker", profile: "cursor-write", prompt: "selected" }), true); assert.equal(Value.Check(tool.parameters, { agent: "worker", prompt: "omitted" }), false); assert.equal(Value.Check(tool.parameters, { profile: "pi-default", prompt: "forged" }), false);
}));
...
void test("mesh_submit rejects omitted, forged, and extra profiles before mutation", async () => withRoot("mesh-profile-premutation-", async root => {
    const mesh = await initializeMesh(root, { rootSessionId: "root", recoverable: true, budgets });
    ...
    const tool = createMeshSubmitTool(deps, localCatalog.roles, { worker: ["pi-default", "cursor-write"], explorer: ["pi-default"] });
    for (const [params, pattern] of [
        [{ agent: "worker", prompt: "omitted" }, /requires an explicit profile/u],
        [{ agent: "worker", profile: "forged", prompt: "forged" }, /not allowed/u],
        [{ agent: "explorer", profile: "pi-default", prompt: "extra" }, /fixed profile/u],
...
void test("mesh_submit reuse requires the current role and selected-profile edge", async () => withRoot("mesh-reuse-profile-edge-", async root => {
    ...
    await assert.rejects(createMeshSubmitTool(deps, { worker: settledAgentDefinition("worker") }, { worker: ["cursor-write"] }).execute("reuse-stale-profile", { agentId: worker.agentId, prompt: "must not reuse" }, undefined, undefined, { cwd: root } as never), /current immutable role\/profile edge/u);
...
void test("reviewer first submit unlocks all peers transactionally and reload restores them", async () => withRoot("mesh-staged-submit-", async root => {
    ...
    const currentEpoch = await readPolicyEpoch(root, mesh.meshId, epoch.epochId); const directDeps = { ...files, env, exec: liveExec, activeCaller: () => caller(mesh.meshId, currentEpoch, { identity: "agent:reviewer", agentId: reviewer.agentId, envelope: reviewer.envelope, endpointId: `agent:${reviewer.agentId}`, sessionFile }) }; const before = await readdir(meshPaths(root, mesh.meshId).tasks); const failing = createMeshSubmitTool(directDeps, { "review-lens": settledAgentDefinition("review-lens"), validator: settledAgentDefinition("validator") }, { "review-lens": ["pi-default"], validator: ["pi-default"] }, activeCaller => activateMeshPeerToolsForSubmit(pi as never, activeCaller, async () => { throw new Error("status persistence failed"); }));
...
void test("post-start steer and followUp signals do not unlock a fresh outbound child", async () => withRoot("mesh-post-start-signals-", async root => {
    ...
    const endpoint = await readMeshEndpoint(root, mesh.meshId, `agent:${reviewer.agentId}`);
    for (const delivery of ["steer", "followUp"] as const) {
        ...
        const priorMessages = pi.messages.length; for (let attempt = 0; attempt < 5 && pi.messages.length === priorMessages; attempt += 1) await tick();
...
// Given two materialization passes and one pending peer before a shared delivery deadline, the endpoint receives one completion message containing both sources and the current pending frontier while signals remain individual.
void test("delivery pump bundles completion sources and preserves the pending frontier", async () => withRoot("mesh-completion-bundle-", async root => {
    ...
    const endpoint = await readMeshEndpoint(root, mesh.meshId, endpointId); await registerMeshSignal(root, mesh.meshId, { callerEndpointId: "agent:sender", toolCallId: "between-sources", endpoint, delivery: "followUp", topic: "status", text: "individual", canonicalArguments: { topic: "status" } });
    ...
    const completionMessages = pi.messages.filter(item => (item.message as any).details?.kind === "completion"); const signalMessages = pi.messages.filter(item => (item.message as any).details?.kind === "signal");
    assert.equal(completionMessages.length, 1); assert.equal(signalMessages.length, 1);
    ...
    assert.doesNotMatch(message.content, /prompt|output|error|usage/u); assert.deepEqual(completionMessages[0]!.options, { deliverAs: "steer", triggerTurn: false }); assert.deepEqual(signalMessages[0]!.options, { deliverAs: "followUp", triggerTurn: false });
    await pi.handlers.get("session_shutdown")![0]!({ reason: "reload" });
}));
