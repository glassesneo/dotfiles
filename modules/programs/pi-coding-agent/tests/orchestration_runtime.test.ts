...
void test("core tool schemas and execution require exactly one selector at each single-target boundary", async () => {
    ...
    assert.equal(Value.Check(submit.parameters, { agent: "worker", prompt: "work" }), true);
    assert.equal(Value.Check(submit.parameters, { agent: "worker", prompt: "work", channel: "A" }), false);
