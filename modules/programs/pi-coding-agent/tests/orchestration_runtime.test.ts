...
void test("mesh_send intervenes when busy, submits when idle, and retries idempotently", async () => withRoot("mesh-send-state-aware-", async root => {
    ...
    assert.deepEqual(JSON.parse((first.content[0] as { text: string }).text), protocolDetails); assert.deepEqual({ agentId: displayIdentity.agentId, role: displayIdentity.role, profile: displayIdentity.profile }, { agentId: worker.agentId, role: "worker", profile: "pi-default" });
