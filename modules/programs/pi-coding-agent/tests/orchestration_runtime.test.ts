...
const role = (name: string): RoleDefinition => ({ description: `Synthetic ${name}`, tools: ["read"], skillOptIns: [], instructions: `Perform ${name}.`, defaultProfile: "pi-default", contextPolicy: name === "gyaru" ? "prompt-only" : "project", childExtensionContributions: name === "reviewer" ? [AGENT_ARTIFACT_EXTENSION] : [] });
