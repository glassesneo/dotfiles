...
void test("overrides replace an action and generated help contains every key", () => {
    const map = resolveQuestionKeymap(manager, { "question.single": { "select-and-note": ["alt+e", "ctrl+e"] } });
...
void test("configuration rejects unknown data, conflicts, and required action removal", () => {
    ...
    assert.throws(() => resolveQuestionKeymap(manager, { "question.single": { accept: ["e"] } }, "/tmp/extension-keybindings.json"), /conflicts.*accept, select-and-note/);
    ...
    assert.doesNotThrow(() => resolveQuestionKeymap(manager, { "question.write-in": { clear: ["ctrl+x"], cancel: [] } }));
});
