...
void test("generated question keymap satisfies the context and action grammar", () => {
    ...
    assert.doesNotThrow(() => resolveQuestionKeymap(loaded.config, loaded.path));
});

void test("overrides replace an action and generated help contains every key", () => {
    const map = resolveQuestionKeymap({ "question.single": { "select-and-note": ["alt+e", "ctrl+e"] } });
    const item = detailedQuestionHelp("question.single", map).find(entry => entry.action === "select-and-note");
...
void test("configuration rejects unknown data, conflicts, and required action removal", () => {
    ...
    assert.throws(() => resolveQuestionKeymap({ "question.single": { accept: ["e"] } }, "/tmp/extension-keybindings.json"), /conflicts.*accept, select-and-note/);
    assert.throws(() => resolveQuestionKeymap({ "question.common": { cancel: ["ctrl+shift+x"] }, "question.single": { accept: ["shift+ctrl+x"] } }), /conflicts.*cancel, accept/);
    assert.throws(() => resolveQuestionKeymap({ "question.text": { accept: [] } }), /required action missing/);
    assert.throws(() => validateQuestionKeymapConfig({ "question.single": { "edit-note": ["e"] } }), /unknown action/);
    assert.doesNotThrow(() => resolveQuestionKeymap({ "question.write-in": { clear: ["ctrl+x"], cancel: [] } }));
});
