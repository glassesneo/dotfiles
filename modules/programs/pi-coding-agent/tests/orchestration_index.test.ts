...
import { bindMeshEndpoint, registerMeshReport } from "../extensions_src/utilities/orchestration_events.ts";
...
void test("reconciliation rebuilds all classes, removes invalid evidence, and preserves crash references", async () => withRoot("mesh-index-reconcile-", async root => {
    ...
    await writeFile(join(meshPaths(root, mesh.meshId).events, `${eventId}.json`), JSON.stringify({ schemaVersion: 1, meshId: mesh.meshId, eventId, endpointId: completion.endpointId, endpointSessionFile: completion.endpointSessionFile, endpointBindingId: completion.bindingId, senderEndpointId: completion.endpointId, senderEndpointSessionFile: completion.endpointSessionFile, delivery: "steer", state: "pending", kind: "report", payload: { eventId, reportId: eventId, agentId, taskId: createdTask, summary: "indexed" }, createdAt }));
...
// Given a report interrupted after index persistence, when the same tool call retries, the receiver observes the originally reserved event ID exactly once.
void test("event creation retry repairs its index-first crash point", async () => withRoot("mesh-event-index-crash-", async root => {
    const mesh = await initializeMesh(root, { rootSessionId: "root", recoverable: true, budgets }); const endpoint = await bindMeshEndpoint(root, mesh.meshId, { endpointId: `root:${mesh.meshId}`, kind: "root", harness: "pi", sessionId: "root", sessionFile: "/root.jsonl" });
    const agentId = randomUUID(); const agentSession = join(root, "crash-agent.jsonl"); await writeFile(agentSession, "");
    const agentEndpoint = await bindMeshEndpoint(root, mesh.meshId, { endpointId: `agent:${agentId}`, kind: "agent", agentId, harness: "pi", sessionId: "crash-agent", sessionFile: agentSession });
    const taskId = randomUUID();
    const input = { callerEndpointId: agentEndpoint.endpointId, callerEndpointSessionFile: agentEndpoint.sessionFile, toolCallId: "report-crash", endpoint, agentId, taskId, summary: "bounded", canonicalArguments: { summary: "bounded" } };
    await assert.rejects(registerMeshReport(root, mesh.meshId, { ...input, afterEventIndexPersisted: () => { throw new Error("event index crash"); } }), /event index crash/u);
    ...
    assert.deepEqual(await registerMeshReport(root, mesh.meshId, input), { reportId: reservedId }); assert.equal((await readdir(meshPaths(root, mesh.meshId).events)).filter(name => name === `${reservedId}.json`).length, 1);
}));
