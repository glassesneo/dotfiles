...
- Preserve ownership boundaries between skills rather than copying one skill's workflow or artifact schema into another. In particular, `agent-artifact` owns durable artifact formats; `source-implementation`, `implementation-validation`, `orchestrated-review`, and `implementation-lifecycle` own Pi's post-approval source, validation, full-review, and lifecycle procedures.
