...
- Preserve ownership boundaries between skills rather than copying one skill's workflow or artifact schema into another. In particular, `agent-artifact` owns durable artifact formats; `source-implementation`, `implementation-validation`, `adaptive-review`, and `implementation-lifecycle` own Pi's heavy post-approval source, validation, adaptive read-only review, and lifecycle procedures; `lightweight-implementation-lifecycle` owns bounded self-implementation, self-validation, and explicitly requested adaptive solo-review remediation.
