...
description: >-
  ...
  delegated targeted validation, and persist the terminal implementation
...
6. After changes, delegate each bounded validation question to `tester`. The
   handoff must include the approved design path, changed scope or diff
   reference, exactly one validation question, and known risks. Load no tester
   procedure into the handoff; the tester owns `targeted-validation`.
7. Use the returned evidence to decide whether implementation is settled.
...
Repair and request a fresh tester run only when evidence identifies a concrete
implementation regression and the repair remains inside the approved design
and scale contract. Record the failed run, any failure-report path, the repair,
and the fresh result.
